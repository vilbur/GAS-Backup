	...
	*/
	...
	var fillSheets = function(spread_or_sheet) {
		...
		if(spread_or_sheet.match('spread','gi')){
			sheet_folder_ids	= {};
			sheet_filetree	= Sheeter.deleteSheets( true, sheet_main ).sheet;
		}
		...
		for(var f=0; f<item_names.length;f++) {
			...
			/* CREATE SHEET */
			sheet	= Sheeter.createOrGetSheet( getSheetName(folder_id), 2, columns_count).sheet;
			/* SETUP SHEET */
			Sheeter.setRows(2).setColumns(columns_count).setHeaders(row_data, row_options);
		...
		if(spread_or_sheet.match('spread','gi')){
			Sheeter.setSheet(sheet_filetree).setRows(2).setColumns(columns_count).setHeaders(row_data, row_options);
